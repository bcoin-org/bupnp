...
class UPNPService {
   ...
   * @param {Object} options - {leaseDuration: Number, protocol: 'TCP'|'UDP', clientDesc: String, internalHosts: Array[String]}
   * @returns {Promise}
   */

  async addPortMapping(remote, externalPort, internalPort, options) {
    const {leaseDuration, protocol, clientDesc, internalHosts} = options || {};
